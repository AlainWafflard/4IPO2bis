...
class Predator(Fauna):
	...
	def __next__(self):
		...
		if abs(self.position - self._food.position) < 1 :
			...
			self._speed = -1 if self.position > self._food.position else +1
...
class Prey(Fauna):
	...
	def __next__(self):
		...
		if abs(self._position - self.__predator.position) <= 1 :
			...
			self._speed = +1 if self.position > self.__predator.position else -1
			self._position += self._speed * 4
			self._energy -= 4
